...
# Once: compile + profile + export JSON (trn2.3xlarge, LNC=1 TP=4)
COMPILE=1 ./profiler/capture_and_export_qwen.sh

# Re-profile only (reuse compiled NEFFs)
./profiler/capture_and_export_qwen.sh

python3 -m dmsim.cli ingest \
  --profile-dir /dev/shm/traced_model/Qwen1.5-MoE-A2.7B-lnc1-tp4-b1-p128-s256-rtopk_softmax/profile \
  --model-key 307929685239809 \
