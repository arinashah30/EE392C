...
BASE_COMPILER_WORK_DIR = os.environ.get("BASE_COMPILE_WORK_DIR", "/dev/shm/nxd_model_compile")
if not BASE_COMPILER_WORK_DIR.endswith(os.sep):
    BASE_COMPILER_WORK_DIR += os.sep
...
class InferenceRunner:
    ...
    def trace_shard_only(
        ...
        **kwargs,
        ...
        """Shard weights into an already-traced model directory (no neuron-cc)."""
        if traced_model_path is not None and not os.path.exists(traced_model_path):
            os.makedirs(traced_model_path)
        ...
        self.neuron_config.save(traced_model_path)
        model = self.get_model_cls().from_pretrained(self.model_path, self.neuron_config)
        model.compile_shard_only(serialize_base_path=traced_model_path)
