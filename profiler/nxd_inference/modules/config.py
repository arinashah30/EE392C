...
class MoENeuronConfig(NeuronConfig):
    ...
    Router-kernel knobs (added for the Qwen-MoE router benchmark in
    ``moe_demo/qwen_moe/routers/``):

      * ``router_kernel`` (str): name of the swappable router; one of
        ``ROUTER_REGISTRY``. Default ``"topk_softmax"`` reproduces the
        original RouterTopK construction byte-for-byte.
      * ``use_nki_router`` (bool): if True, the Group-A routers (1–5)
        route through the fused NKI shell in
        ``qwen_moe/routers/fused_router_kernel.py`` instead of their
        torch reference path. Has no effect for Group-B routers.
      * ``router_ft_checkpoint`` (Optional[str]): path to a
        ``router_weights.safetensors`` produced by the Tier-T1
        fine-tune (``moe_demo/bench/finetune_router.py``). Loaded into
        the router after construction. Has no effect for routers
        with no learnable parameters (``hash_routing``).
      * ``expert_ft_checkpoint`` (Optional[str]): path to merged expert
        weights (``expert_weights_merged.safetensors`` from
        ``moe_demo/bench/finetune_expert_lora_hash.py``). Overrides HF
        expert ``gate_proj`` / ``up_proj`` / ``down_proj`` before NxD
        packs them into ``expert_mlps``.
      * ``router_kwargs`` (Optional[dict]): per-router escape hatch.
        E.g. ``{"n_group": 12, "topk_group": 4}`` for
        ``group_limited_topk`` or ``{"num_slots": 4}`` for
        ``soft_moe``.
    """
    def __init__(
            self,
            ...
            glu_mlp: bool = True,
            router_kernel: str = "topk_softmax",
            use_nki_router: bool = False,
            router_ft_checkpoint: str = None,
            expert_ft_checkpoint: str = None,
        ...
        self.router_kernel = router_kernel
        self.use_nki_router = bool(use_nki_router)
        self.router_ft_checkpoint = router_ft_checkpoint
        self.expert_ft_checkpoint = expert_ft_checkpoint
