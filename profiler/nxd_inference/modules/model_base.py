...
class NeuronBaseForCausalLM(HuggingFaceGenerationAdapter):
    ...
    def compile(self, serialize_base_path=None):
        ...
        base_compile_work_dir = os.environ.get("BASE_COMPILE_WORK_DIR", "/dev/shm/nxd_model_compile")
        from neuronx_distributed.utils.model_utils import get_platform_lnc

        lnc = int(get_platform_lnc())
    ...
    def compile_shard_only(self, serialize_base_path=None):
        """Write sharded safetensors only (skip trace + neuron-cc).

        Use when ``model.pt`` and NEFFs are already present but weight
        sharding failed or was interrupted. Still needs ~70 GiB host RAM
        briefly while slicing the HF checkpoint.
        """
        base_compile_work_dir = os.environ.get("BASE_COMPILE_WORK_DIR", "/dev/shm/nxd_model_compile")
